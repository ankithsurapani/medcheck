...
- `src/resolve/review_cli.py` — the 0.75–0.92 band, cluster-vs-cluster, **40 of 210 pairs decided (35 approved, 5 rejected), 170 left pending** (see the 2026-08-07 review-session addenda below — the queue grew from 205 to 210 when Ticket 2's PIN-derived states fed the scorer)
...
**Applied state: manufacturers 1,889 → 1,856** (35 approvals + 5 rejections against the rebuilt queue), **collapse ratio 2.73 : 1**. `--apply` refuses to run with undecided review pairs unless given `--allow-pending`; that flag was used deliberately — the 170 still-pending pairs are treated as **not merged** (conservative, can only under-merge). 6,077 of 6,155 records carry a `manufacturer_id`; the 78 unlinked are the 7 placeholder strings (`manufacturer_unknown_placeholder`), which deliberately stay `NULL` — never resolved into a company entity.
...
- **13.5% of companies hold 54.5% of flags** — but **52.7% appear exactly once** and the median company has 1. Long tail with a heavy head, not a few bad actors. (Lower bound: 170 review-band pairs are still undecided — see Phase 2a.)
...
- ~~190 review-band pairs undecided *and* slugs positional~~ — the slug half is **fixed** (above); the review is **underway** — 40 of 210 decided as of 2026-08-07 (`multi_plant` partial, `near_typo` complete, `other` not started, see the addenda above) — **170 pairs still pending**, still its own ticket to finish. The AI executing session is not authorized to decide the remainder unattended (`plan.md` §1) — it needs the user, live, same as every pass so far.
