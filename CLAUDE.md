...
**`failure_category` vocabulary extended (2026-08-06)** — §3.3 gained `ph`, `water_content`, `uniformity_of_weight`, `bacterial_endotoxins`, `uniformity_of_dispersion`. `other` fell from 657 records (10.7%) to 363 (5.9%).
...
- 2026-08-06 — `water_content` deliberately excludes "Loss on Drying" (measures total volatiles, not water) and "Water-soluble substances" (a solubility/impurity test). Both stay in `other`.
...
- **`pH` is the single most common failure with no original §3.3 bucket** — 260 records, more than `disintegration` (234) or `related_substances` (183). Now mapped. `Loss on Drying`, `weight per ml`, `specific gravity`, `length`, `appearance of solution` and `extractable volume` still have no bucket and remain the bulk of the 364 records in `other`.
